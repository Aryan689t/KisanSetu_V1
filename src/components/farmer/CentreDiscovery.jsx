...
import { MapPin, Clock, Users, Calendar, Search, Filter, ShieldCheck, CheckCircle2, Navigation, HelpCircle, ArrowRight } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-agri-ivory-muted">
        <div>
          ...
          <p className="text-xs text-agri-text-muted mt-0.5 font-sans">
            Compare queue congestion, real-time wait times, and get Google Maps directions to optimal procurement hubs.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-2 text-xs text-agri-green-dark bg-agri-green-soft px-3 py-1.5 rounded-xl border border-agri-green-border font-medium">
          ...
          <span>Real-time Mandi Telemetry (DoCA Network)</span>
        </div>
      </div>

      {/* FEATURE HERO: Best Procurement Centre For You (Derived dynamically from live telemetry) */}
      <div className="paper-surface rounded-2xl p-6 border-2 border-agri-gold shadow-agri-md relative space-y-4 bg-gradient-to-r from-agri-gold-light/10 via-[#FFFDF7] to-agri-ivory/50">
        
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-agri-ivory-muted">
          <div>
            <div className="flex items-center space-x-2 flex-wrap gap-y-1 mb-2">
              <span className="bg-agri-gold text-agri-green-dark px-3 py-1 rounded-full text-xs font-extrabold shadow-agri-sm font-mono">
              ...
              {recommendedCentre.id !== bookedCentreId && (
                <span className="bg-agri-ivory text-agri-text text-[11px] font-bold px-2.5 py-0.5 rounded-full border border-agri-ivory-muted font-sans">
                  Current Booking: {centres.find(c => c.id === bookedCentreId)?.name.split(' ')[0] || 'Sonipat'}
                </span>
              )}
            </div>
            
            <h2 className="font-heading text-2xl font-bold text-agri-green flex items-center gap-2">
              {recommendedCentre.name}
            </h2>

            <p className="text-xs text-agri-text-muted flex items-center space-x-1.5 mt-1">
              <MapPin className="w-4 h-4 text-agri-green shrink-0" />
          ...
          <div className="flex items-center space-x-2.5 shrink-0">
            <a
              ...
              className="bg-agri-green-soft hover:bg-agri-green text-agri-green-dark hover:text-white px-3.5 py-2.5 rounded-xl text-xs font-bold inline-flex items-center space-x-1.5 transition-colors border border-agri-green-border shadow-agri-sm"
            ...
            <button
              ...
              className="bg-agri-gold hover:bg-agri-gold-dark text-agri-green-dark font-extrabold text-xs px-5 py-2.5 rounded-xl shadow-agri-sm transition-all flex items-center space-x-1.5 hover:scale-[1.02]"
            >
              <Calendar className="w-4 h-4" />
              <span>Select Centre & Book Slot</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center py-2">
          <div className="bg-agri-ivory/80 p-3 rounded-xl border border-agri-ivory-muted">
            <span className="text-[10px] text-agri-text-muted uppercase font-bold block">Est. Waiting Time</span>
            <p className="font-heading text-xl font-extrabold text-agri-gold-dark mt-0.5 font-mono">~{recommendedCentre.estWaitMinutes} min</p>
          </div>

          <div className="bg-agri-ivory/80 p-3 rounded-xl border border-agri-ivory-muted">
            <span className="text-[10px] text-agri-text-muted uppercase font-bold block">Current Queue</span>
            <p className="font-heading text-xl font-extrabold text-agri-text mt-0.5 font-mono">{recommendedCentre.queueCount} Farmers</p>
          </div>

          <div className="bg-agri-ivory/80 p-3 rounded-xl border border-agri-ivory-muted">
            <span className="text-[10px] text-agri-text-muted uppercase font-bold block">Yard Capacity Load</span>
            <p className="font-heading text-xl font-extrabold text-agri-green mt-0.5 font-mono">{recommendedCentre.capacityPercent}%</p>
          </div>

          <div className="bg-agri-ivory/80 p-3 rounded-xl border border-agri-ivory-muted">
            <span className="text-[10px] text-agri-text-muted uppercase font-bold block">Available Slots</span>
            <p className="font-heading text-xl font-extrabold text-agri-green mt-0.5 font-mono">{recommendedCentre.availableSlots} Open</p>
          </div>
        </div>
        ...
        <div className="pt-2">
          ...
          {showWhyHero && (
            <div className="mt-3 p-4 bg-agri-ivory/90 rounded-xl border border-agri-ivory-muted text-xs text-agri-text space-y-2 animate-in fade-in duration-200">
        ...
        {filteredCentres.map((centre) => {
          ...
          return (
            ...
            >
              ...
              {/* Badges Ribbon */}
              <div className="absolute top-0 right-6 -translate-y-1/2 flex items-center space-x-1 font-mono text-[10px] font-extrabold uppercase">
                {isBooked && (
              ...
              {/* Action Bar */}
              <div className="pt-4 border-t border-agri-ivory-muted flex items-center justify-between gap-2">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-agri-ivory hover:bg-agri-ivory-muted text-agri-green-dark px-3 py-2 rounded-xl text-xs font-bold inline-flex items-center space-x-1 border border-agri-ivory-muted transition-colors"
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-extrabold transition-all shadow-agri-sm flex items-center space-x-1.5 hover:scale-[1.02] ${
                    isRec
