...
const DirectionsButton = ({ centre, style = 'outline' }) => {
  ...
  return (
    <a
      ...
      className={`rounded-xl text-xs font-bold inline-flex items-center justify-center space-x-1.5 touch-target min-h-[44px] transition-all ${
        style === 'solid'
    ...
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div>
        <h1 className="font-heading text-xl sm:text-2xl font-bold text-agri-text">
          📍 {t('अपनी फसल के लिए मंडी चुनें', 'Where should I go to sell my crop?')}
        </h1>
      ...
      <div className="bg-[#17432A] text-white rounded-2xl p-5 sm:p-6 shadow-agri-md relative space-y-4 border-2 border-agri-gold">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/10 pb-3">
          <div>
            <span className="bg-agri-gold text-agri-green-dark px-3 py-0.5 rounded-full text-xs font-extrabold shadow-sm font-mono inline-block mb-1">
              <Star className="w-3 h-3 inline-block -mt-0.5 mr-0.5" /> {t('आपके लिए सबसे अच्छा विकल्प', 'Best option for you')}
            </span>
          ...
          <span className="bg-emerald-900/80 text-emerald-200 px-3 py-1 rounded-full border border-emerald-500/40 font-bold text-xs self-start sm:self-auto">
            🟢 {t('कम इंतजार', 'Less waiting')}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 text-center">
          ...
          <div className="bg-[#102e1c] p-3 rounded-xl border border-agri-gold/20">
            <span className="text-[11px] text-agri-ivory/70 block">
              📅 {t('स्लॉट उपलब्ध', 'Slots available')}
            </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
          ...
          >
            ...
            <span>{t('यह मंडी बुक करें', 'Book this Mandi')}</span>
          </button>
        </div>
          ...
          <div className="flex items-center justify-between border-b border-agri-ivory-muted pb-2.5">
            <div>
              <span className="text-[11px] font-bold text-agri-green bg-agri-green-soft px-2.5 py-0.5 rounded-full border border-agri-green-border font-sans inline-block mb-1">
                <CheckCircle2 className="w-3 h-3 inline-block -mt-0.5 mr-0.5" /> {t('आपकी बुक की गई मंडी', 'Your booked mandi')}
              </span>
            ...
            <div className="text-right font-mono">
              <span className="text-[10px] text-agri-text-muted uppercase block">{t('टोकन पास', 'Token Pass')}</span>
                  ...
                  <div key={centre.id} className="p-4 rounded-xl border border-agri-ivory-muted bg-[#FFFDF7] space-y-2.5">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${
                        ...
                        {centre.capacityPercent > 80 ? t('🔴 भारी भीड़', '🔴 Very busy') : t('🟢 कम भीड़', '🟢 Less waiting')}
                      </span>
                    </div>

                    <div className="grid grid-cols-2 gap-2 text-center text-xs bg-agri-ivory/50 p-2 rounded-lg">
                      <div>
                        <span className="text-[10px] text-agri-text-muted block">{t('इंतजार समय', 'Wait time')}</span>
                        <strong className="text-agri-gold-dark font-mono mt-0.5 block">~{centre.estWaitMinutes} min</strong>
                      </div>
                      <div>
                        <span className="text-[10px] text-agri-text-muted block">{t('स्लॉट उपलब्ध', 'Slots available')}</span>
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-1">
                      ...
                      <button
                        ...
                        className="bg-agri-green hover:bg-agri-green-dark text-white py-2 rounded-xl text-xs font-bold flex items-center justify-center space-x-1 shadow-sm touch-target min-h-[40px]"
                      >
